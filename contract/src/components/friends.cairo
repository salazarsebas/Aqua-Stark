...
pub mod FriendState {
    ...
    use dojo_starter::entities::base::{FriendRequestSent, FriendRequestAccepted, FriendRequestRejected, FriendRequestDeleted};
    ...
    impl FriendsImpl of IFriendState<ContractState> {
        fn get_friend_request(self: @ContractState, request_id: u64, player: Player) -> FriendRequest{
        ...
        fn reject_friend_request(ref self: ContractState, request_id: u64, player: Player){
            ...
            // Emit event
            let request_event = FriendRequestRejected { request_id, player: player.id, sender: request.sender };
            world.emit_event(@request_event);
        }
