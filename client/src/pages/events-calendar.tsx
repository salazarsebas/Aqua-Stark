...
export default function EventsCalendar() {
  ...
  // Find featured event for the current tab
  // const featuredEvent =
  //   filteredEvents.find((event) => event.featured) || (activeTab !== "all" ? filteredEvents[0] : null)
  ...
  return (
    <div className="relative min-h-screen flex flex-col bg-gradient-to-b from-blue-500 via-blue-700 to-blue-900 text-white">
      ...
      {/* Header */}
      <nav className="relative z-10 p-4 bg-blue-700 border-b-2 border-blue-400/50 px-4 xl:px-24">
        <div className="flex items-center justify-between mx-auto font-sans max-w-7xl">
          <div className="flex items-center">
            <button className="mr-2 text-xs text-white rounded-full hover:bg-blue-500/50 flex items-center">
      ...
      <div className="relative z-10 p-4 flex items-center justify-between w-full gap-x-2 md:gap-x-4 px-4 xl:px-16">
        <div className="relative w-full">
          <input
            type="text"
            placeholder="Search events..."
            className="bg-blue-800/50 w-full flex rounded-md pl-2 md:pl-10 pr-4 py-2 flex-1 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 text-gray-200 placeholder:text-gray-300"
            value={searchQuery}
            onChange={(e) => setSearchQuery(e.target.value)}
          />
          <Search className="absolute right-3 top-1/2 transform -translate-y-1/2 w-4 h-4 text-blue-300" />
        </div>
        <div className="flex items-center gap-2">
          <button
            className={`flex items-center gap-1 px-1 md:px-3 py-2.5 md:py-2 rounded-lg text-xs md:text-sm w-20 md:w-28 ${
          ...
          <button
            className={`flex items-center gap-1 px-1 md:px-3 py-2.5 md:py-2 rounded-lg text-xs md:text-sm w-28 md:w-36 ${
      ...
      <div className="relative z-10 mb-4 px-4 xl:px-16 hidden md:block">
        <div className="bg-blue-700 rounded-lg p-1 flex">
          <button
            className={`flex-1 flex items-center justify-center gap-1 py-2 rounded-md text-sm font-medium ${
          ...
          <button
            className={`flex-1 flex items-center justify-center gap-1 py-2 rounded-md text-sm font-medium ${
              activeTab === "tournaments" ? "bg-blue-600" : ""
            }`}
      ...
      {/* Content Area */}
      <div className="relative z-10 px-4 xl:px-16 pb-16 flex-grow">
        {viewType === "list" && (
          <>
            {activeTab === "all" ? (
